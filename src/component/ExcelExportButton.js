...
const ExcelExportButton = ({ data, defaultFileName = 'export', allColumns }) => {
  ...
  const exportToExcel = () => {
    ...
    const worksheet = workbook.addWorksheet('Data');
         worksheet.columns = Object.keys(filteredData, {
      header: selectedColumns.map(key => headerMap[key][0] || {}).map(key => ({ header: key, key: key }));
    ...
    // Create workbook
    const workbook = new ExcelJS.Workbook();// Worksheet already added to workbook above// Generate Excel file
