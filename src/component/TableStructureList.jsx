          ...
          <tbody>
            {currentItems.map((row, index) => (
              <tr key={index}>
                ...
                <td>
                  {row.status === "1" || user?.employeeid === row.user_id || user?.role === "Admin" ? (
                    <Link
                      to={`/view/${slug}`}
                      state={{ id: row.id }}
...
export default React.memo(TableStructureList);